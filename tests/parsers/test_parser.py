...
from nomad_measurements_dls.schema_packages.schema_package import RawFileDLSData, ELNMalvernZmes


def test_is_mainfile():
    """Test that the parser correctly identifies .zmes files and rejects others."""
    parser = DLSParser()
    assert parser.is_mainfile("sample.zmes", "application/octet-stream", b"dummy_data", "") is True
    assert parser.is_mainfile("SAMPLE.ZMES", "application/octet-stream", b"dummy_data", "") is True
    assert parser.is_mainfile("sample.zmes", "application/octet-stream", b"", "") is False
