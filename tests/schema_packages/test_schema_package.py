...
def test_eln_normalization(mock_read_zmes):
    ...
    assert entry.instrument_setup.scattering_angle.magnitude == 173.0 # noqa PLR2004
    ...
    assert result_data.temperature.magnitude == 25.0 # noqa PLR2004
    assert result_data.measurement_type == 'Size (DLS)'

    assert np.array_equal(
        result_data.correlation_lag_times.magnitude,
        np.array([1, 2, 3], dtype=np.float64),
    )
    assert np.array_equal(
        result_data.correlation_data, np.array([0.9, 0.5, 0.1], dtype=np.float64)
    )
